...
#pragma omp atomic
					...
					for (int i = 0; i < traversed_cells[tid].size(); ++i)
					{
						...
						// TODO TASK 3: DVR in front-to-back order
						if (actual_cell.FrontToBack(ray, cell_hit.t_0, cell_hit.t_1, &sample_color, &sample_alpha)) break;
						//if (actual_cell.BackToFront(ray, cell_hit.t_0, cell_hit.t_1, &sample_color, &sample_alpha)) break;

					}
					lambert = lambert * lambert_alpha * Vector3(0.51f, 0.48f, 0.43f);
					pixel_color += (lambert * 0.5f);
